...
* [TEST] No absorb
	...
	* 1. Run benchmark
	poisson `lhs' `rhs', vce(robust)
	notrim
	matrix list e(V)
	storedresults save benchmark e()

	* 2. Run ppmlhdfe
	ppmlhdfe `lhs' `rhs', tol(1e-12) itol(1e-12) maxiter(50)
	notrim
	matrix list e(V)
...
* [TEST] No absorb against GLM
	loc lhs price
	loc rhs weight gear length
	loc absvars c

	* 1. Run benchmark
	glm `lhs' `rhs', vce(robust) fam(poisson) irls
	notrim
	loc glm_deviance = e(deviance)
	storedresults save benchmark e()

	* 2. Run ppmlhdfe
	ppmlhdfe `lhs' `rhs', absorb(`absvars')
	notrim
	storedresults compare benchmark e(), tol(1e-8) include(`list_poisson')
	assert inrange(e(ic), 1, 1000)
	...
	di  `glm_deviance'
	di  e(deviance)
	di  reldif(`glm_deviance', e(deviance)) 
	assert reldif(`glm_deviance', e(deviance)) < 1e-10

* [TEST] Absorb turn; robust SEs
	...
	* 2. Run ppmlhdfe
	ppmlhdfe `lhs' `rhs', absorb(`absvars') vce(robust) keepsingletons tol(1e-12) v(-1)
	trim_cons
	storedresults compare benchmark e(), tol(1e-8) include(`list_poisson')
...
* [TEST] Absorb turn; clustered SEs
	...
	* 2. Run ppmlhdfe
	ppmlhdfe `lhs' `rhs', absorb(`absvars') vce(cluster `absvars') keepsingletons tol(1e-12)  v(-1)
	trim_cons
